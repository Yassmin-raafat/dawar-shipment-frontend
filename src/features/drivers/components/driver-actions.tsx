...
export default function DriverActions({ driver }: { driver: Driver }) {
  ...
  return <>
    <button type="button" aria-label={"Actions for " + driver.name} popoverTarget={menuId}
      onClick={(event) => {
    ...
    <div ref={menuRef} id={menuId} popover="auto" style={position} className="fixed m-0 w-44 rounded-xl border border-border bg-card p-1.5 text-left text-xs text-text-primary shadow-lg">
      <Link href={href + "?edit=true"} onClick={() => menuRef.current?.hidePopover()} className="block rounded-lg px-3 py-2.5 hover:bg-secondary focus-visible:outline-primary">View/Edit</Link>
      <Link href={href + "?assignShipment=true"} onClick={() => menuRef.current?.hidePopover()} className="block rounded-lg px-3 py-2.5 text-primary hover:bg-primary-muted focus-visible:outline-primary">Assign Shipment</Link>
    </div>
  </>;
}

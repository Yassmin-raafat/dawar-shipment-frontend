...
export default function MessageInput({ recipient, conversationId }: { recipient: string; conversationId: string }) {
  ...
  return <form onSubmit={submit} aria-busy={mutation.isPending} className="shrink-0 border-t border-border/30 bg-card px-3 py-3">
    <div className="flex items-center gap-2 rounded-2xl border border-[#edf1f6] bg-[#f5f7fb] p-1.5">
      <button type="button" aria-label="Attach file (coming soon)" aria-disabled="true" title="Attachments are coming soon" className="grid size-8 shrink-0 place-items-center rounded-lg text-[#91a3bd]">
      ...
      <label className="min-w-0 flex-1"><span className="sr-only">Message to {recipient}</span><input ref={inputRef} value={draft} readOnly={mutation.isPending} aria-describedby={showError ? "send-message-error" : undefined} onChange={(event) => { setDrafts((current) => ({ ...current, [conversationId]: event.target.value })); if (mutation.isError) mutation.reset(); }} onKeyDown={(event) => { if (event.key === "Enter" && event.nativeEvent.isComposing) event.preventDefault(); }} placeholder={`Type your message to ${recipient}...`} className="h-8 w-full min-w-0 bg-transparent text-[11px] text-text-primary outline-none placeholder:text-[#91a3bd] focus-visible:ring-2 focus-visible:ring-primary/20" /></label>
      <button type="button" aria-label="Choose emoji (coming soon)" aria-disabled="true" title="Emoji picker is coming soon" className="grid size-7 shrink-0 place-items-center rounded-lg text-[#91a3bd]">
      ...
      <button type="submit" disabled={!draft.trim() || mutation.isPending} className="inline-flex h-8 shrink-0 items-center gap-1.5 rounded-xl bg-primary px-3 text-[11px] font-medium text-primary-foreground disabled:cursor-not-allowed disabled:opacity-60">{mutation.isPending ? "Sending..." : "Send"}
    ...
    {showError && <p id="send-message-error" role="alert" className="mt-2 text-[11px] text-destructive">{mutation.error.message || "Could not send message. Please try again."}</p>}
  </form>;
}

  ...
  return <section aria-labelledby="messages-title" className="flex h-full min-h-0 flex-col rounded-2xl bg-card">
    <div className="shrink-0 border-b border-border/20 px-4 pb-4 pt-4">
      <div className="mb-3 flex items-center justify-between gap-3">
        ...
        <button type="button" onClick={onNewChat} aria-label={t("newChat")} title={t("newChat")} className="grid size-7 place-items-center rounded-lg bg-[#f5f7fa] text-[#52647e]">
      ...
      <label className="relative block"><span className="sr-only">{t("searchConversations")}</span>
        ...
        <input type="search" value={search} onChange={(event) => onSearch(event.target.value)} placeholder={t("searchPlaceholder")} className="h-8 w-full rounded-xl border border-transparent bg-[#f5f7fa] ps-8 pe-3 text-[10px] text-text-primary outline-none placeholder:text-[#91a3bd] focus:border-primary/40" />
      </label>
    </div>
